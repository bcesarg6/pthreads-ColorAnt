...
/* Sort a list of solutions based on the number of conflicts */
void sortSolutions(gcp_solution_t **solutions){
    /* Implementado um insertion sort por se tratar de um número pequeno */
    /* de elementos (max = número de nucleos totais da maquina). Mudar?  */
    int i,j;
    gcp_solution_t *aux;


    for(i = 1; i < n_threads; i++){
        for(j = i; j > 0  && solutions[j]->nof_confl_vertices < solutions[j-1]->nof_confl_vertices; j--){
            //Troca solutions[j-1] por solutions[j]
            aux = solutions[j-1];
